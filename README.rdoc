szz: This is a clone of the project voteable mongo from: https://github.com/vinova/voteable_mongo, however a little bit of code was added to support votes on embedded collections up to n-level of embedding (making use of Mongo DB metadata). Collections holding embedded collections needs to define "accepts_nested_attributes_for" for the embedded collection accepting votes, besides this everything must work as expected following the next instructions.
